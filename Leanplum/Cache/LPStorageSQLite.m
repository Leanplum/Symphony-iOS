...
@implementation LPStorageSQLite
...
- (sqlite3 *)initSQLite
{
    const char *sqliteFilePath = [[LPStorageSQLite sqliteFilePath] UTF8String];
    ...
    // Create tables.
    [self runQuery:@"CREATE TABLE IF NOT EXISTS requests ("
     "data TEXT NOT NULL,"
...
- (void)handleSQLiteError:(NSString *)errorName errorResult:(int)result query:(NSString *)query
{
    ...
    if (result == SQLITE_CORRUPT && !retryOnCorrupt) {
        [[NSFileManager defaultManager] removeItemAtPath:[LPStorageSQLite sqliteFilePath] error:nil];
        retryOnCorrupt = YES;
        [self initSQLite];
    }
}
                               ...
                               bindObjects:(NSArray *)objectsToBind
{
    ...
    [objectsToBind enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        if (![obj isKindOfClass:[NSString class]]) {
            NSLog(@"Bind object have to be NSString.");
        }
    ...
    @synchronized (self) {
        @try {
            ...
            if (result != SQLITE_DONE) {
                NSLog(@"SQLite fail to run query.");
            }
            sqlite3_finalize(statement);
        } @catch (NSException *e) {
            NSLog(@"SQLite operation failed.");
